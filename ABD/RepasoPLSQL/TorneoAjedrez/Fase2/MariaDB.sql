...
CREATE TABLE arbitros
	(
		...
		CONSTRAINT codigo_arbitro_valido		CHECK (codigo REGEXP '^[A-D]{1}[0-9]{3}[A-Z]{2}$')
	);
...
CREATE TABLE torneos
	(
		...
		CONSTRAINT edicion_valida 	CHECK (edicion REGEXP '^[0-9]{1,3}ª$')
	);
...
CREATE TABLE movimientos
	(
		...
		CONSTRAINT nombre_casilla_valido 	CHECK ((casilla_salida AND casilla_llegada) REGEXP '^[A-H]{1}[1-8]{1}$')
	);