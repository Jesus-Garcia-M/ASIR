...
(
	...
	CONSTRAINT CH_DNI CHECK(DNI ~ '[0-9]{8}[A-Z]' OR DNI ~ '[K,L,M,X,Y,Z][0-9]{7}[A-Z]')
);
...
(
	...
	CONSTRAINT CH_CODP CHECK(CODIGO SIMILAR TO'[A-Z][0-9]{4}-[A-Z]'),
...
(
	...
	CONSTRAINT CH_COD CHECK(CODIGO ~ '^[A-Z0-9]*')
);
...
(
	...
	CONSTRAINT CH_VAL CHECK(VALORACION IN('CUMPLIDO', 'TEMPORALMENTE NO EVALUABLE') OR TO_NUMBER(VALORACION,'9') BETWEEN '0' AND '100'),
...
(
	...
	CONSTRAINT CH_DATECOL CHECK((TO_CHAR(FECHAINI, 'DD') BETWEEN '01' AND '07') AND (TO_CHAR(FECHAINI, 'DAY') LIKE '%MONDAY%'))
);
